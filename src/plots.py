...
def save_all_outputs(config, prompt_1_images, prompt_2_images, blend_images, blend_method, output_path):
    ...
    intermediate_image_blend = prompt_1_images[config["from_timestep"]]
...
def make_plots(image_1, image_2, image_blend, prompt_1, prompt_2, timesteps, additional_parameters, output_path):
    ...
    ax[1].set_title(f"{prompt_1}-BLEND-{prompt_2}-{additional_parameters}")
    ax[1].axis("off")
    ...
    plt.savefig(f"{output_path}/blending-{additional_parameters}-{prompt_1}-BLEND-{prompt_2}.png")
...
def make_blending_batch_grid(output_paths, blend_method, config):
    ...
    save_path = os.path.join(save_path, f"results-[{blend_method}]-[{additional_parameters}]-[{model_id}]-[{scheduler_name}]")
